...
class Dilateformer_T(nn.Module):
    def __init__(self, pretrained=True):
        super().__init__()
        self.model = Dilateformer(depths=[2, 2, 6, 2], embed_dim=72, num_heads=[ 3, 6, 12, 24 ])
        if pretrained==True:
            self.load_from()
    def forward(self, rgb):
        bottle_feature, downsample_list = self.model(rgb)
        return bottle_feature, downsample_list

    def load_from(self, ):
        pretrained_path = "checkpoints/dilate_tiny.pth"
        ...
        print("---start load pretrained modle of Dilateformer encoder---")
...
if __name__ == "__main__":
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    x = torch.rand([2,3,512,1024]).to(device)
    m = Dilateformer_T(pretrained=True).to(device)
    y = m(x)
    print(y)
