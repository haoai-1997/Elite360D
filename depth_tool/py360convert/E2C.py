...
class Equirec2Cube:
    ...
    def _xyz2coor(self):
        ...
        # longitude and latitude to equirectangular coordinate
        self.coor_x1 = (lon / (2 * np.pi) + 0.5) * 2 - 1
        self.coor_y1 = (-lat / np.pi + 0.5) * 2 - 1
        self.coord_cp = np.concatenate([self.coor_x1 * np.pi, self.coor_y1 * np.pi * 0.5], axis=-1)
    ...
    def run(self, equ_img, equ_dep=None):
        ...
        if equ_dep is not None:
            return cube_img, cube_dep,self.coord_cp
        else:
            return cube_img,self.coord_cp